...
# --- إعدادات آمنة ---
TOKEN = os.environ.get('TOKEN')  # التوكن من Environment Variables
...
# --- دوال المؤشرات البديلة (بدون مكتبة ta) ---
...
def calculate_macd(prices):
    ...
    macd = ema_12 - ema_26
    signal = calculate_ema([macd] * 9, 9)  # إشارة مبسطة
    return round(macd, 4), round(signal, 4)

def extract_indicators_simple(df):
    """استخراج المؤشرات بدون مكتبة ta"""
    if df.empty:
        return {
            "rsi": 50, "macd": 0, "signal": 0, "sma": 0, "ema": 0,
            "bb_upper": 0, "bb_lower": 0, "stoch_k": 50, "stoch_d": 50,
            "adx": 25, "cci": 0, "volume": 0, "willr": -50, "obv": 0,
            "atr": 0.001, "last_price": 0
        }
    ...
    # Bollinger Bands مبسطة
    if len(prices) >= 20:
        period_prices = prices[-20:]
        sma_bb = sum(period_prices) / len(period_prices)
        variance = sum((x - sma_bb) ** 2 for x in period_prices) / len(period_prices)
        std_dev = variance ** 0.5
        bb_upper = round(sma_bb + 2 * std_dev, 4)
        bb_lower = round(sma_bb - 2 * std_dev, 4)
    else:
        bb_upper = round(prices[-1] * 1.02, 4) if prices else 0
        bb_lower = round(prices[-1] * 0.98, 4) if prices else 0
    ...
    return {
        ...
        "bb_upper": bb_upper,
        "bb_lower": bb_lower,
        "stoch_k": 50,  # قيمة افتراضية
        "stoch_d": 50,  # قيمة افتراضية
        "adx": 25,      # قيمة افتراضية
        "cci": 0,       # قيمة افتراضية
        "volume": df.get("volume", pd.Series([0] * len(df))).iloc[-1] if not df.empty else 0,
        "willr": -50,   # قيمة افتراضية
        "obv": 0,       # قيمة افتراضية
        "atr": 0.001,   # قيمة افتراضية
        "last_price": last_price,
    }

# --- دوال تحليل السوق المعدلة ---
    ...
    async with aiohttp.ClientSession() as session:
        for key in API_KEYS_TWELVE:
            try:
                ...
                indicators = extract_indicators_simple(df)
...
async def duration_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    BOT_STATS["unique_users_set"] = list(unique_users_set)
...
def main():
    ...
    print("🤖 بوت هياف يعمل الآن على Render بنجاح! 🚀")
    app.run_polling()
